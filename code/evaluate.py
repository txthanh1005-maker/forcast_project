...
def main():
    ...
    rf_preds = rf_model.predict(X_test_tab)
    ...
    lstm_checkpoint = torch.load(os.path.join(models_dir, "lstm_model.pth"))
    ...
    with torch.no_grad():
        ...
        lstm_preds = linear(out[:, -1, :]).numpy().flatten()
    ...
    start_idx = 0
    end_idx = min(start_idx + window_size, len(true_vals))
    ...
    plt.plot(time_axis, lgbm_plot[start_idx:end_idx], label='LightGBM', linestyle='--', alpha=0.8)
    plt.plot(time_axis, xgb_plot[start_idx:end_idx], label='XGBoost', linestyle='--', alpha=0.8)
    plt.plot(time_axis, lstm_plot[start_idx:end_idx], label='LSTM', linestyle='-', alpha=0.8)
    
    plt.title('Model Predictions vs Actual Utilization (7-Day Window)')
    plt.xlabel('Hours')
    plt.ylabel('Utilization Rate')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(figures_dir, "predictions_comparison_7days.png"), dpi=300)
    plt.close()
