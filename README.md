...
- [**`Latex_report/sn-article.tex`**](Latex_report/sn-article.tex): **TRÁI TIM CỦA DỰ ÁN**. Đây là bản thảo (manuscript) hoàn chỉnh của bài báo khoa học. File này chứa đựng:
  ...
  - Bộ sưu tập 5 Siêu biểu đồ Vector mã hóa bằng `TikZ` & `pgfplots` (Kiến trúc Proxy-Lag, Time-Series Split, TPT Graph, Optuna Flowchart, XAI Decision Tree) nâng tầm trực quan dữ liệu.
